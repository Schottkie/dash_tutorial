...
@app.callback(
    [Output('plot_lex', 'figure'),
     Output('plot_birth_death', 'figure')],
...
def show_selected_country(checklist_indicators_value, dropdown_countries_value, rangeslider_lex_value):
    ...
    for country in dropdown_countries_value:
        ...
        fig_birth_death.add_trace(go.Scatter(
            x=df_country['MidPeriod'], y=df_country['Births'], name='Births'), row=1, col=1
        )
        fig_birth_death.add_trace(go.Scatter(
            x=df_country['MidPeriod'], y=df_country['Deaths'], name='Deaths'), row=1, col=2
        )
